...
import { Loader2, ArrowLeft, AlertCircle, CheckCircle2, Download, X, ArrowUpDown, ArrowUp, ArrowDown, FileSpreadsheet, FileText, FileSearch } from "lucide-react";
...
export default function TrackICMSPage() {
  ...
  const [activeTab, setActiveTab] = useState<string>(searchParams.get("tab") || "missing");
  ...
  useEffect(() => {
    const loadExams = async () => {
      try {
        ...
        // Only validate exam_id from URL if provided - don't auto-select
        if (selectedExamId && allExams.length > 0) {
          const examExists = allExams.some(e => e.id === selectedExamId);
          if (!examExists) {
            // Exam from URL doesn't exist, clear selection
            setError(`Exam with id ${selectedExamId} not found.`);
            setSelectedExamId(null);
            // Update URL to remove invalid exam_id
            router.replace(`/icm-studio/track-icms`, { scroll: false });
          } else {
            setError(null); // Clear error if exam is valid
          }
        }
      } catch (error) {
  ...
  // Validate selected exam when exams list changes
  useEffect(() => {
    if (exams.length > 0 && selectedExamId) {
      const examExists = exams.some(e => e.id === selectedExamId);
      if (!examExists) {
        setError(`Exam with id ${selectedExamId} not found.`);
        // Clear invalid exam selection
        setSelectedExamId(null);
        // Update URL to remove invalid exam_id
        ...
        router.replace(`/icm-studio/track-icms${params.toString() ? `?${params.toString()}` : ""}`, { scroll: false });
      } else {
        setError(null);
      }
    }
  }, [exams, selectedExamId, router, selectedSchoolId, selectedSubjectId, selectedTestType, selectedSubjectType]);
  ...
  useEffect(() => {
    const loadMissingSheets = async () => {
      ...
      try {
        // Verify exam exists before making the request (only if exams are loaded)
        if (exams.length > 0) {
          const examExists = exams.some(e => e.id === selectedExamId);
          if (!examExists) {
            throw new Error(`Exam with id ${selectedExamId} not found.`);
          }
        }
        ...
        // Filter sheets by subject_type if selected (for all sheet types)
        ...
        setMissingSheets(filterBySubjectType(result.missing_sheet_ids_info));
        setError(null); // Clear error on success
      } catch (error) {
        console.error("Error loading missing sheets:", error);
        const errorMessage = error instanceof Error ? error.message : "Failed to load missing sheets.";
        if (errorMessage.includes("not found")) {
          setError(errorMessage);
          // Clear the invalid exam selection
          setSelectedExamId(null);
          setComparison(null);
          setMissingSheets([]);
          // Update URL to remove invalid exam_id
    ...
    loadMissingSheets();
  }, [selectedExamId, selectedSchoolId, selectedSubjectId, selectedTestType, selectedSubjectType, subjects, exams, router]);
  ...
  useEffect(() => {
    ...
    router.replace(`/icm-studio/track-icms?${params.toString()}`, { scroll: false });
  ...
  // Get all sheet types (filtered by subject type if selected)
  const getFilteredSheets = useMemo(() => {
    if (!comparison) return { missing: [], uploaded: [], expected: [], extra: [] };
    ...
    return {
      missing: filterBySubjectType(comparison.missing_sheet_ids_info),
      uploaded: filterBySubjectType(comparison.uploaded_sheet_ids_info),
      expected: filterBySubjectType(comparison.expected_sheet_ids_info),
      extra: filterBySubjectType(comparison.extra_sheet_ids_info),
    };
  }, [comparison, selectedSubjectType, subjects]);
  ...
  const exportToCSV = () => {
    ...
    const selectedExam = exams.find(e => e.id === selectedExamId);
    const examLabel = selectedExam ? `${selectedExam.exam_type}_${selectedExam.series}_${selectedExam.year}` : "track_icms";
    const dateStr = new Date().toISOString().split("T")[0];
    link.setAttribute("download", `track_icms_${examLabel}_${dateStr}.csv`);
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      const examLabel = selectedExam ? `${selectedExam.exam_type}_${selectedExam.series}_${selectedExam.year}` : "track_icms";
      const dateStr = new Date().toISOString().split("T")[0];
      const filename = `track_icms_${examLabel}_${dateStr}.xlsx`;
          ...
          <div className="container mx-auto px-6 py-8 space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold tracking-tight">Track ICMS</h1>
                  <p className="text-muted-foreground">
                    Track and monitor missing score sheets that were expected but have not been uploaded yet
                  </p>
                </div>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      ...
                      // Verify exam exists before setting
                      const examExists = exams.some(e => e.id === examId);
                      if (!examExists) {
                ...
                <div className="lg:col-span-1">
                  <Card className="sticky top-4">
                    <CardHeader>
                      ...
                      <CardDescription>Filter sheets by school, subject, test type, and subject type</CardDescription>
                    </CardHeader>
                ...
                <div className="lg:col-span-3">
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          <CardTitle>Sheet Tracking Details</CardTitle>
                          <CardDescription>
                            Detailed breakdown of expected vs uploaded score sheets
                            {comparison && ` (${comparison.total_uploaded_sheets} uploaded of ${comparison.total_expected_sheets} expected)`}
                          </CardDescription>
                        </div>
                        {activeTab === "missing" && filteredAndSortedSheets.length > 0 && (
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      <TabsList className="grid w-full grid-cols-4">
                        <TabsTrigger value="missing">
                          Missing ({getFilteredSheets.missing.length})
                        </TabsTrigger>
                        <TabsTrigger value="uploaded">
                          Uploaded ({getFilteredSheets.uploaded.length})
                        </TabsTrigger>
                        <TabsTrigger value="expected">
                          Expected ({comparison.total_expected_sheets})
                        </TabsTrigger>
                        <TabsTrigger value="extra">
                          Extra ({getFilteredSheets.extra.length})
                        </TabsTrigger>
                      </TabsList>

                      {/* Missing Sheets Tab */}
                      <TabsContent value="missing" className="space-y-4 mt-4">
                        {getFilteredSheets.missing.length === 0 ? (
                          <div className="py-12 text-center">
                      ...
                      <TabsContent value="uploaded" className="space-y-4 mt-4">
                        <div className="rounded-md border">
                          <Table>
                            ...
                            <TableBody>
                              {getFilteredSheets.uploaded.length === 0 ? (
                                <TableRow>
                                ...
                                getFilteredSheets.uploaded.map((info) => (
                      ...
                      <TabsContent value="expected" className="space-y-4 mt-4">
                        <div className="rounded-md border">
                          <Table>
                            ...
                            <TableBody>
                              {getFilteredSheets.expected.length === 0 ? (
                                <TableRow>
                                ...
                                getFilteredSheets.expected.slice(0, 100).map((info) => (
                                  <TableRow key={info.sheet_id}>
                                    <TableCell className="font-mono text-sm">{info.sheet_id}</TableCell>
                                    <TableCell>
                                      <Badge variant="outline">
                          ...
                          {getFilteredSheets.expected.length > 100 && (
                            <div className="p-4 text-sm text-muted-foreground text-center">
                              Showing first 100 of {getFilteredSheets.expected.length} expected sheets
                            </div>
                          )}
                        </div>
                      </TabsContent>
                      ...
                      <TabsContent value="extra" className="space-y-4 mt-4">
                        <div className="rounded-md border">
                          <Table>
                            ...
                            <TableBody>
                              {getFilteredSheets.extra.length === 0 ? (
                                <TableRow>
                                  <TableCell colSpan={5} className="text-center text-muted-foreground">
                                ...
                                getFilteredSheets.extra.map((info) => (
                                  <TableRow key={info.sheet_id}>
                                    <TableCell className="font-mono text-sm">{info.sheet_id}</TableCell>
                                    <TableCell>
                                      <Badge variant="outline">

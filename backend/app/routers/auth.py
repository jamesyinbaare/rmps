...
async def register(
    ...
    # Note: Register endpoint doesn't return tokens - user must login separately
...
async def refresh_token(
    refresh_request: RefreshTokenRequest, session: DBSessionDep
) -> TokenRefreshResponse:
    ...
    # Get user
    user_stmt = select(User).where(User.id == refresh_token_db.user_id)

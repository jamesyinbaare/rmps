...
import {
  displayBankCode,
  getPublicWorkforceBankAccount,
  getPublicWorkforceDistinctBankNames,
  listPublicWorkforceBankBranches,
  upsertPublicWorkforceBankAccount,
  type WorkforceBankAccountPublic,
} from "@/lib/api";
...
const FORM_ID = "workforce-bank-account-form";
...
function accountDisplayValue(row: WorkforceBankAccountPublic): string {
  return resolveBankKind(row.bank_name) === "absa"
    ? splitAbsaAccountForDisplay(row.account_number, row.bank_code)
    : row.account_number;
}

export function WorkforceBankAccountForm({ kind, token, examinationLabel, className }: Props) {
  const formId = useId();
  ...
  const [saved, setSaved] = useState<WorkforceBankAccountPublic | null>(null);
  ...
  const [branchRows, setBranchRows] = useState<
    Awaited<ReturnType<typeof listPublicWorkforceBankBranches>>["items"]
  >([]);
  ...
  const loadAccount = useCallback(async () => {
    ...
    setCardError(null);
    try {
      const row = await getPublicWorkforceBankAccount(kind, token);
      setSaved(row);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const names = await getPublicWorkforceDistinctBankNames(kind, token, bankNameQuery.trim());
  ...
  }, [bankNameQuery, kind, modalOpen, token]);

  useEffect(() => {
    if (!modalOpen || !selectedBankName.trim()) {
      setBranchRows([]);
      return;
    }
    let cancelled = false;
    setBranchesLoading(true);
    void listPublicWorkforceBankBranches(kind, token, {
  ...
  }, [kind, modalOpen, selectedBankName, token]);

  function openModal() {
    setFormError(null);
    if (saved) {
      setSelectedBankName(saved.bank_name);
      setBankNameQuery(saved.bank_name);
      setSelectedBranchId(saved.bank_branch_id);
      const acct = accountDisplayValue(saved);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setBusy(true);
    try {
      const row = await upsertPublicWorkforceBankAccount(kind, token, {
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-base font-semibold text-foreground">Bank account for payment</h2>
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
              {saved
                ? "Your payment will be sent to the account below."
                : "Add the bank account where your payment should be sent."}
            </p>
          </div>
        </div>
        {cardError ? (
        ...
        {successMessage ? (
          <p className="mt-4 rounded-lg border border-emerald-500/30 bg-emerald-500/10 px-3 py-2 text-sm text-foreground" role="status">
            {successMessage}
          </p>
        ) : null}
        {saved ? (
        ...
        <Button type="button" className="mt-4 min-h-11 w-full" onClick={openModal} disabled={busy}>
          {saved ? (
            <>
        ...
        <OfficialModal
          ...
          footer={
            <div className={officialModalFooterClass()}>
              <button type="button" className={officialAccountsBtnSecondary} onClick={closeModal} disabled={busy}>
                Cancel
              </button>
              <button
                type="submit"
            ...
            <FormSection title="Bank account details" description={accountFieldCopy.description}>
              ...
              {showBankFields ? (
                <div className="space-y-4">
                  <div>
                    <label className={formLabelClass} htmlFor={`${formId}-bank`}>
                      Bank name
                    </label>
                  ...
                  <div>
                    <label className={formLabelClass} htmlFor={`${formId}-branch`}>
                      Branch
                    </label>
                    <SearchableCombobox
                      id={`${formId}-branch`}
              ...
              {showAccountFields ? (
                <div className="space-y-4">
                  ...
                  <div>
                    <label className={formLabelClass} htmlFor={`${formId}-acct-confirm`}>
                      Confirm account number
                    </label>
                    <input
                      id={`${formId}-acct-confirm`}
                      className={`${formInputClass} font-mono tracking-wide`}

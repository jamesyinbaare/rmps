"""Script checker assignment batches."""
...
from app.services.workforce_assignment_batches import (
    ...
    cancel_script_checker_assignment_batch,
    complete_script_checker_assignment_batch,
    create_script_checker_assignment_batch,
...
@router.get(
    "/examinations/{examination_id}/script-checker-assignments/roster",
    response_model=WorkforceAssignmentRosterResponse,
)
async def get_script_checker_assignment_roster(
...
async def get_script_checker_assignments(
    session: DBSessionDep,
    ...
    try:
        data = await list_script_checker_assignment_grid(
            session,
...
async def create_script_checker_assignment(
    session: DBSessionDep,
    user: SuperAdminOrTestAdminOfficerOrSubjectOfficerDep,
    examination_id: int,
    subject_id: int,
    ...
    await assert_subject_officer_access(session, user, examination_id, subject_id)
    try:
        row = await create_script_checker_assignment_batch(
            session,
            examination_id=examination_id,
            subject_id=subject_id,
            paper_number=paper_number,
...
@router.post(
    "/examinations/{examination_id}/subjects/{subject_id}/script-checker-assignments/{batch_id}/complete",
    response_model=WorkforceAssignmentBatchRow,
)
async def complete_script_checker_assignment(
    session: DBSessionDep,
    user: SuperAdminOrTestAdminOfficerOrSubjectOfficerDep,
    examination_id: int,
    subject_id: int,
    ...
    await assert_subject_officer_access(session, user, examination_id, subject_id)
    try:
        row = await complete_script_checker_assignment_batch(
            session,
            examination_id=examination_id,
            subject_id=subject_id,
            batch_id=batch_id,
...
@router.post(
    "/examinations/{examination_id}/subjects/{subject_id}/script-checker-assignments/{batch_id}/cancel",
    response_model=WorkforceAssignmentBatchRow,
)
async def cancel_script_checker_assignment(
    session: DBSessionDep,
    ...
    try:
        row = await cancel_script_checker_assignment_batch(
            session,
            examination_id=examination_id,
            subject_id=subject_id,
            batch_id=batch_id,

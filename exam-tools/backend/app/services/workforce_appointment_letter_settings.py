"""Per-examination + workforce-kind appointment letter signatory and CC settings."""
...
from app.models import (
    ...
    ExaminationWorkforceAppointmentLetterSettings,
    WorkforceKind,
)
...
from app.services.examiner_appointment_letter_settings import (
    ...
    DEFAULT_DIRECTOR_GENERAL_TITLE,
    DEFAULT_VALEDICTION,
)
...
def _resolved_dg_name(row: ExaminationWorkforceAppointmentLetterSettings | None) -> str:
    if row is None:
        return ""
    return _trim(row.director_general_name)


def _resolved_dg_title(row: ExaminationWorkforceAppointmentLetterSettings | None) -> str:
    if row is None:
        return DEFAULT_DIRECTOR_GENERAL_TITLE
    return _trim(row.director_general_title) or DEFAULT_DIRECTOR_GENERAL_TITLE


def _resolved_dac_name(row: ExaminationWorkforceAppointmentLetterSettings | None) -> str:
    if row is None:
        return DEFAULT_DIRECTOR_ASSESSMENT_NAME
    return _trim(row.director_assessment_name) or DEFAULT_DIRECTOR_ASSESSMENT_NAME


def _resolved_dac_title(row: ExaminationWorkforceAppointmentLetterSettings | None) -> str:
    if row is None:
        return DEFAULT_DIRECTOR_ASSESSMENT_TITLE
    return _trim(row.director_assessment_title) or DEFAULT_DIRECTOR_ASSESSMENT_TITLE


def _signing_official(row: ExaminationWorkforceAppointmentLetterSettings | None) -> AppointmentLetterSigningOfficial:
    if row is None:
...
def _signed_for_dg(row: ExaminationWorkforceAppointmentLetterSettings | None) -> bool:
    if row is None:
        return True
    return bool(row.signed_for_director_general)


def _resolved_valediction(row: ExaminationWorkforceAppointmentLetterSettings | None) -> str:
    if row is None:
        return DEFAULT_VALEDICTION
    return _trim(row.valediction) or DEFAULT_VALEDICTION


def resolve_letter_date(row: ExaminationWorkforceAppointmentLetterSettings | None) -> date | None:
...
def require_letter_date_for_pdf(row: ExaminationWorkforceAppointmentLetterSettings | None) -> datetime:
...
) -> ExaminationWorkforceAppointmentLetterSettings | None:
    return await session.get(
        ExaminationWorkforceAppointmentLetterSettings,
        (examination_id, _kind_enum(kind)),
    )
...
async def get_or_create_settings(
    ...
    row = await get_settings_row(session, examination_id, kind)
    if row is not None:
        return row
    row = ExaminationWorkforceAppointmentLetterSettings(
        examination_id=examination_id,
        kind=_kind_enum(kind),
...
def settings_to_response(
    examination_id: int,
    kind: WorkforceKind | str,
    row: ExaminationWorkforceAppointmentLetterSettings | None,
) -> WorkforceAppointmentLetterSettingsResponse:
    official = _signing_official(row)
    kind_enum = _kind_enum(kind)
    return WorkforceAppointmentLetterSettingsResponse(
        examination_id=examination_id,
        kind=WorkforceAppointmentLetterKindApi(kind_enum.value),
        ...
        reference_number=_trim(row.reference_number) if row is not None else "",
...
def _legacy_signature_src() -> str | None:
    from app.services.examiner_appointment_letter_settings import LEGACY_SIGNATURE_REL_PATH

    app_dir = Path(__file__).parent.parent
    if (app_dir / LEGACY_SIGNATURE_REL_PATH).is_file():
        return LEGACY_SIGNATURE_REL_PATH
    return None

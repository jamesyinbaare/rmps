...
from app.schemas.workforce import ScriptCheckerRatesPut
...
@pytest.mark.asyncio
async def test_put_script_checker_rates_upserts_ot_st_rates() -> None:
    session = AsyncMock()
    exam = MagicMock()
    session.get = AsyncMock(side_effect=[exam, None])
    ...
    assert result["objective_rate_per_script_ghs"] == Decimal("0.40")
    assert result["subjective_rate_per_script_ghs"] == Decimal("0.50")
    assert result["rate_per_script_ghs"] == Decimal("0.40")
